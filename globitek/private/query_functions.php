<?php
  ...
  function validate_user($user, $noPW=false, $errors=array()) {
    ...
    if(!$noPW) {
      if (is_blank($user['password'])) {
        ...
        $errors[] = "Password needs to be at least 12 characters long";
      } elseif (!has_valid_password_format($user['password'])) {
        $errors[] = "Password needs to contain at least one of each: uppercase letter, lowercase letter, letter, symbol";
      }

      if (is_blank($user['password_confirmation'])) {
        $errors[] = "Confirmation password cannot be blank.";
      } elseif (!has_length($user['password_confirmation'], array('min' => 12))) {
         $errors[] = "Confirmation password needs to be at least 12 characters long";
      } elseif (!has_valid_password_format($user['password_confirmation'])) {
        $errors[] = "Confirmation password needs to contain at least one of each: uppercase letter, lowercase letter, letter, symbol";
      }
  ...
  function insert_user($user) {
    ...
    $errors = validate_user($user, false);
    if (!empty($errors)) {
      return $errors;
    }
    ...
    $user['hashed_password'] = password_hash($user['password'], PASSWORD_BCRYPT, $options);
  ...
  function update_user($user) {
    ...
    // checking for no password update
    if ($user['password'] === "" && $user["password_confirmation"] === "") {
      $errors = validate_user($user, true);
    } else {
  ...
  function delete_user($user) {
   ...
   // find_failed_login('rockclimber67');
  function find_failed_login($username='') {
    global $db;
    $sql = "SELECT * FROM failed_logins ";
    $sql .= "WHERE username ='" . db_escape($db, $username['username']) . "';";
    $failed_attempt_result = db_query($db, $sql);
    return $failed_attempt_result;
  }

  // insert failed login
  function insert_failed_login($login_info) {
    global $db;

    $sql = "INSERT INTO failed_logins ";
    $sql .= "(username, count, last_attempt) ";
    $sql .= "VALUES (";
    $sql .= "'" . db_escape($db, $login_info['username']) . "',";
    $sql .= "'" . db_escape($db, strval($login_info['count'])) . "',";
    $sql .= "'" . db_escape($db, $login_info['last_attempt']) . "'";
    $sql .= ");";
    // For INSERT statements, $result is just true/false
    $result = db_query($db, $sql);
    if($result) {
  ...
  // update failed login
  function update_failed_login($login_info) {
    global $db;

    $sql = "UPDATE failed_logins SET ";
    $sql .= "username='" . db_escape($db, $login_info['username']) . "', ";
    $sql .= "count='" . db_escape($db, strval($login_info['count'])) . "', ";
    $sql .= "last_attempt='" . db_escape($db, $login_info['last_attempt']) . "' ";
    $sql .= "WHERE id='" . db_escape($db, $login_info['id']) . "' ";
    $sql .= "LIMIT 1;";
    // For update_user statements, $result is just true/false
    $result = db_query($db, $sql);
    if($result) {
  ...
  // Trottle control - failed database
